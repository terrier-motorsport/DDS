    ...
    def __define_devices(self):
        ...
        if self.I2C_ENABLED:
            ...
            self.devices['coolingLoopSensors'] = ADS_1015("Cooling loop", logger=self.logFile, i2c_bus=self.i2c_bus, inputs = [
                Analog_In('hotPressure', 'bar', mapper=M3200_value_mapper, tolerance=0),           #ADC1(A0)
                Analog_In('hotTemperature', '°C', mapper=NTC_M12_value_mapper, tolerance=0),       #ADC1(A1)
                Analog_In('coldPressure', 'bar', mapper=M3200_value_mapper, tolerance=0),          #ADC1(A2)
                Analog_In('coldTemperature', '°C', mapper=NTC_M12_value_mapper, tolerance=0)       #ADC1(A3)
            ])
